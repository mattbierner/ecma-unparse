...
var stream = require("nu-stream")["stream"],
    ...
    joins = (function(arr, joiner) {
        return ((!arr.length) ? NIL : ((arr.length === 1) ? arr[0] : stream.append(arr[0], stream.append(joiner,
            joins(arr.slice(1), joiner)))));
    }),
...
addRewrite("ForStatement", (function(node) {
    ...
    return ((node.init && (node.init.type === "VariableDeclaration")) ? forDeclarationStatement(init, test,
        update, body) : forStatement(init, test, update, body));
}));
...
addRewrite("ObjectSetter", (function(node) {
    return objectSetExpression(_unparse(node.key), _unparse(node.value.params), _unparse(node.value.body));
}));
