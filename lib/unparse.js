...
var identifier = function(name) {
    return seq(new token.IdentifierToken(null, name));
}

var string = function(value) {
    return seq(new token.StringToken(null, value));
};

var number= function(value) {
    return seq(new token.NumberToken(null, value));
};

var nil = function(value) {
    return seq(new token.NullToken(null, value));
};

var boolean = function(value) {
    return seq(new token.BooleanToken(null, value));
};

var regexp = function(value) {
    return seq(new token.RegularExpressionToken(null, value));
};
...
// Expression
    ...
    case 'ObjectExpression':
        return objectExpression(node.properties.map(function(x) {
            switch (x.kind) {
            case 'get':
                return objectGetExpression(_unparse(x.key), _unparse(x.value.body));
            case 'set':
                return objectSetExpression(_unparse(x.key), x.value.params.map(_unparse), _unparse(x.value.body));
            case 'init':
            default:
                return objectValueExpression(_unparse(x.key), _unparse(x.value));
            }
        }));
